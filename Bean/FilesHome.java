...
public class FilesHome
{
	...
	public Files findById(int id)
	{
		...
		try
		{
			Files instance = (Files) sessionFactory.getCurrentSession().get("deefw.Files", id);
			if (instance == null)
			{
	...
	public List<Files> findByExample(Files instance)
	{
		...
		try
		{
			List<Files> results = (List<Files>) sessionFactory.getCurrentSession().createCriteria("deefw.Files")
